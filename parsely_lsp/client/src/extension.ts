...
export async function activate(context: ExtensionContext) {
    let serverModule = workspace.getConfiguration('par').get<string>('lsPath')
    let debug = workspace.getConfiguration('par').get<boolean>('debug')
...
async function promptLSPath(): Promise<string> {
    ...
    workspace.getConfiguration('par').update('lsPath', value, ConfigurationTarget.Global)
    return value
}
